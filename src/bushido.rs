...
impl Plugin for BushidoPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(InputManagerPlugin::<PlayerAction>::default())
            ...
            .add_systems(
                Startup,
                (create_player, background_setup.after(set_up_windows)),
            )
            .add_systems(
                Update,
                (animate_sprites, player_sprite_states, player_update),
            );
    }
}
...
fn player_update(
    ...
    let (mut transform, top_state, mut bottom_state, mut facing, mut moving, mut mover) =
        player.single_mut();
...
fn player_slash(time: Res<Time>, top_state: Query<&mut PlayerTopState>) {}
